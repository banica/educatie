%Nume program: Teodorescu2.m
%Implementarea algorimului FFT bazat pe segmentarea semnalului in timp
%Autor : Teodorescu Florentina Raluca
%Grupa: 343 AB
%Data ultimei modificari : 28.08.2008
%Rutine apelate: gen_arma.m, st_arma.m, densitate_spectrala.m, fft_t.m
...
figure(7);
unitx=linspace(0,1,length(Xfft_t));
subplot(3,1,1);
fft_t_serie=10*log10(abs(Xfft_t));
plot(unitx,fft_t_serie,'b');
axis tight;
grid on;
title('Spectrul seriei de timp evaluat cu algoritmul FFT bazat pe segmentarea semnalului in timp');
...
subplot(3,1,3);
eroare_spec=fft_t_serie-fft_serie;
...
figure(9);
unitx2=linspace(0,1,length(Zgfft_t));
subplot(3,1,1);
fft_t_zg=10*log10(abs(Zgfft_t));
plot(unitx2,fft_t_zg,'b');
axis tight;
grid on;
title('Spectrul zgomotului colorat evaluat cu algoritmul FFT bazat pe segmentarea semnalului in timp');
%xlabel('Frecventa normalizata');
ylabel('Spectru [dB]');
hold on;
...
subplot(3,1,3);
eroare_spec_zg=fft_t_zg-fft_zg;
...
figure(8);
subplot(3,1,1);
fft_t_sf=angle(Xfft_t);
plot(unitx,fft_t_sf,'b');
axis tight;
grid on;
title('Faza seriei de timp evaluata cu algoritmul FFT bazat pe segmentarea semnalului in timp');
%xlabel('Frecventa normalizata');
ylabel('Spectru [rad]');
hold on;
...
subplot(3,1,3);
eroare_spec_faza=fft_t_sf-fft_sf;
...
figure(10);
subplot(3,1,1);
fft_t_zgf=angle(Zgfft_t);
plot(unitx2,fft_t_zgf,'b');
axis tight;
grid on;
title('Faza zgomotului colorat evaluat cu algoritmul FFT bazat pe segmentarea semnalului in timp');
%xlabel('Frecventa normalizata');
ylabel('Faza [rad]');
hold on;
...
subplot(3,1,3);
eroare_spec_zg_faza=fft_t_zgf-fft_zgf;
